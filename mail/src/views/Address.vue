      ...
      <div class="container">
        <div class="checkout-addr">
          ...
          <div class="addr-list-wrap">
            <div class="addr-list">
              <ul>
                <li :class="{'check':addressChecked == index}" @click="addressChecked=index;selectAddressId=item.addressId" v-for="(item,index) in addressListFilter">
                  <dl>
                    <dt>{{item.userName}}</dt>
                  ...
                  <div class="addr-opration addr-del">
                    <a href="javascript:;" class="addr-del-btn" @click="delAddressConfirm(index)">
                      <svg class="icon icon-del"><use xlink:href="#icon-del"></use></svg>
                    </a>
                  </div>
                  <div class="addr-opration addr-set-default" v-if="!item.isDefault">
                    <a href="javascript:;" class="addr-set-default-btn" @click="setDefault(item)"><i>Set default</i></a>
                  </div>
          ...
          <div class="next-btn-wrap">
            <router-link class="btn btn--m btn--red" v-bind:to="{path:'order',query:{'addressId':selectAddressId}}">Next</router-link>
          </div>
        </div>
      </div>
    </div>
    <modal v-bind:mdShow="mdShowAddr" v-on:close="closeDelAddrModal">
      <p slot="msg">
        您确认删除此条地址信息？
      </p>
      <div slot="btn-group">
        <a href="javascript:;" class="btn btn--m" @click="delAddress">确认</a>
        <a href="javascript:;" class="btn btn--m" @click="mdShowAddr=false">关闭</a>
      </div>
    </modal>
...
<script>
  ...
  export default {
    data () {
      return {
        selectAddressId:'',//保存选中的地址ID
        addressList:[],//保存地址列表信息
        limit:3,      //保存要展现的地址列表长度
        addressChecked:0, //选中项
        mdShowAddr:false,//控制删除地址模态框显示隐藏
        delAddressId:''//将要删除的地址 的index
      }
    },
    computed:{
      addressListFilter(){
    ...
    methods: {
      getAddressList(){
        axios.get('/users/addressList').then(response=>{ //获取地址列表信息
          this.addressList = response.data.result
          this.addressList.forEach((item,index)=>{ //将默认地址提取到第一位
            if(item.isDefault){
              var address = this.addressList.splice(index,1)
              this.addressList.unshift(address[0])
              this.selectAddressId = item.addressId //初始化选择地址ID
              return;
            }
          })
        })
      ...
      delAddressConfirm(addressId){
        this.mdShowAddr = true
        this.delAddressId = addressId
      },
      delAddress(){
        axios.post('/users/delAddress',{
          addressId:this.addressList[this.delAddressId].addressId
        }).then(response=>{
          let res = response.data
          if(res.status == '0'){
            this.addressList.splice(this.delAddressId,1)
            this.mdShowAddr = false
          }
        })
      },
      closeDelAddrModal(){
