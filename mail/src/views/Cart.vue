<template>
  <div>
              ...
              <li v-for="(item,index) in cartList">
                  ...
                  <div class="item-quantity">
                    <div class="select-self select-self-open">
                      <div class="select-self-area">
                        <a class="input-sub" @click="cartEdit('-',item)">-</a>
                        <span class="select-ipt">{{item.productNum}}</span>
                        <a class="input-add" @click="cartEdit('+',item)">+</a>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div class="cart-tab-5">
                  <div class="cart-item-opration">
                    <a href="javascript:;" class="item-edit-btn" @click="confirmDeleteModal(index)">
    ...
    <modal v-bind:mdShow="mdShowDel" v-on:close="closeDel">
      <p slot="msg">确认删除本商品？</p>
      <div slot="btn-group">
        <a href="javascript:;" class="btn btn--m" @click="confirmDelete">确认</a>
...
<script>
  ...
  export default {
    data () {
      return {
        cartList: [],//保存购物车商品数据
        mdShowDel: false,//删除模态框显示隐藏
        delIndex: '', // 将要删除商品的index
      }
    },
    computed: {
      checkAll(){ //购物车 商品 全选
      ...
      totalPrice(){
        var money = 0
        this.cartList.forEach((item) => {
          if (item.checked == '1') money += item.salePrice * item.productNum
        })
        return money
      }
    },
    ...
    methods: {
      ...
      confirmDeleteModal(index){ // 弹出删除模态框
        this.mdShowDel = true
        this.delIndex = index
      },
      confirmDelete(){ //确认删除
        axios.post('/users/cartDel', {
          productId: this.cartList[this.delIndex].productId
        }).then(response => {
          let res = response.data
          if (res.status == '0') {
            this.mdShowDel = false
            var count = this.cartList[this.delIndex].productNum
      ...
      cartEdit(type, item){ //购物车内商品数量 增加 减少
        if (type == '+') {
          item.productNum++
